...
class KnowledgeBaseGenerator:
    ...
    def generate_complete_knowledge_base(self, data_schema: Dict) -> Dict:
        ...
        # Resume from a previously saved knowledge base if one exists; otherwise
        # start from an empty base and generate every level from scratch.
        saved_knowledge_path = self.output_dir / "complete_knowledge_base_with_question.json"
        if saved_knowledge_path.exists():
            with open(saved_knowledge_path, 'r') as f:
                saved_complete_knowledge = json.load(f)
        else:
            saved_complete_knowledge = {
                "pairwise_knowledge": None,
                "group_knowledge": None,
                "overall_knowledge": None,
                "narrative_knowledge": None,
            }
