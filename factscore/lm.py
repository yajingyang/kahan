...
class LM(object):
    ...
    def generate(self, prompt, cache_key, sample_idx=0, max_sequence_length=6000, max_output_length=128):
        prompt = prompt.strip() # it's important not to end with a whitespace
        cache_key = f"{cache_key}_{sample_idx}"
    ...
    def get_cache_result(self, cache_key: str, sample_idx=0):
        cache_key = f"{cache_key}_{sample_idx}"
        return self.cache_dict.get(cache_key, None)
