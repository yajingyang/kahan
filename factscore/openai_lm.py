...
openai.api_version = os.getenv("OPENAI_API_FACTUALITY_API_VERSION")
...
class OpenAIModel(LM):
    ...
    def _generate(self, prompt, max_sequence_length=2048, max_output_length=128):
        ...
        elif self.model_name in ["gpt-4o-mini", "o1-mini", "o3-mini"]:
            # Construct the prompt send to GPT-4o / o1-mini
            message = [{"role": "user", "content": prompt}]
            # Call API
            response = call_GPT4o(message, model_name=self.model_name, temp=self.temp, max_len=max_sequence_length)
            # Get the output from the response
            output = response["choices"][0]["message"]["content"]
            return output, response, prompt
...
def call_GPT4o(message, model_name="gpt-4o", max_len=1024, temp=0.7, verbose=False):
    # call GPT-4O API until result is provided and then return it
    response = None
    received = False
    num_rate_errors = 0
    while not received:
