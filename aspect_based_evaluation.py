...
class DnAEvaluator:
    ...
    def evaluate(self, context: str,  candidates: List[str]) -> Tuple[List[float], int]:
        ...
        for aspect in self.criteria.aspects:
            scores = self.score_candidates(context, candidates, aspect)
...
if __name__ == "__main__":
    ...
    evaluator = DnAEvaluator(query_llm=get_gpt_response, aspect_context=criteria_aspect_context, weight_context=criteria_weight_context)
    ...
    # Edit this list to match the model(s) / ablations you generated narratives for.
    setup_list = [
        ...
        {"model_name": "gpt-4o", "with_entity_analysis_knowledge": False, "with_insight_processing_knowledge": False, "with_narrative_knowledge": False, "with_hierarchical_structure": True, "question_based_insight_processing_knowledge": False, "prompting_postfix": ""},
        {"model_name": "gpt-4o", "with_entity_analysis_knowledge": True, "with_insight_processing_knowledge": True, "with_narrative_knowledge": True, "with_hierarchical_structure": False, "question_based_insight_processing_knowledge": True, "prompting_postfix": ""},
    ...
    for market_dir in market_dir_list:
        ...
        for data_path in data_path_list:
            ...
            update_json(cur_results, "results/evaluation_results_randomized.json")
    df = pd.DataFrame(results)
    df.to_csv("results/evaluation_results_randomized.tsv", sep="\t", index=False)
    agg_results = df.groupby(by=list(setup_list[0].keys()))[["score", "aspect_0", "aspect_1", "aspect_2"]].mean().reset_index()
    agg_results.to_csv("results/evaluation_results_agg_randomized.csv", index=False)
